...
let displayLS = parseInt(sessionStorage.getItem("displ")) || 0;
...
parseInt(sessionStorage.setItem("displ", displayLS));
...
attack.onmousedown = () => {
  ...
  if (enemyHP.innerHTML <= 0) {
    info.innerHTML = "It looks like this one is not gona spawn ever again.....";
    clearInterval(interv);
    pomucka--;
...
displayButt.onclick = () => {
  displayLS += 1;
  displaing.style.display = "none";
  parseInt(sessionStorage.setItem("displ", displayLS));
  displayButt.style.display = "none";
};
